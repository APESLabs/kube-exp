...
export default function StatefulSet() {
    ...
    const syncResources = async (namespace: string) => {
        const response = await KubeAPI.listStatefulSet(namespace);
    ...
    const handleDelete = (namespace: string, name: string) => {
        (async () => {
            const response = await KubeAPI.deleteStatefulSet(namespace, name);
            Notification.open({
    ...
    const columns = [
        ...
        },
        {
            title: '状态',
            render: (text: any, record: any, index: number) => {
                const { availableReplicas, replicas } = record.status;
                return (
