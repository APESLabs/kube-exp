...
import { listDeployments, deleteDeployment } from './kube-api'
...
export default function Deployment() {
    ...
    const syncResources = async (namespace: string) => {
        const response = await listDeployments(namespace);
    ...
    const handleDelete = (namespace: string, name: string) => {
        (async () => {
            const response = await deleteDeployment(namespace, name);
            Notification.open({
